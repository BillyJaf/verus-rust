...
struct_with_invariants!{
    ...
    {
        invariant on atomic with (cell) is (v: bool, g: Option<PointsTo<Nil>>) {
            match g {
                None => v == true,
                Some(points_to) => {
                    v == false &&
...
impl LockedNil {
    ...
    fn insert_with_no_cdr(&self, mut nil_perm: Tracked<PointsTo<Nil>>, insert_car: u32) -> ((updated_nil_perm, new_cons_perm): (Tracked<PointsTo<Nil>>, Tracked<PointsTo<Cons>>))
        requires
            self.wf(),
            nil_perm.is_init(),
            ...
            new_cons_perm.value().car == insert_car
    {
        ...
        return (nil_perm, locked_cons_perm)
    }

    fn insert_with_cdr(&self, mut nil_perm: Tracked<PointsTo<Nil>>, cons_perm: &Tracked<PointsTo<Cons>>, insert_car: u32) -> ((updated_nil_perm, new_cons_perm): (Tracked<PointsTo<Nil>>, Tracked<PointsTo<Cons>>))
        requires
            self.wf(),
            nil_perm.is_init(),
            cons_perm.is_init(),
            ...
            self.wf()
    {
        ...
        if (nil_view.cdr.is_none()) {
            let (mut updated_nil_perm, new_cons_perm) = self.insert_with_no_cdr(nil_perm, insert_car);
            ...
            if (insert_car < first_cons_view.car) {
                ...
                let (mut updated_nil_perm, new_cons_perm) = self.insert_with_cdr(nil_perm, &first_cons_perm, insert_car);
                let updated_nil_view = self.cell.borrow(Tracked(updated_nil_perm.borrow_mut()));
                let new_locked_cons = updated_nil_view.cdr.as_ref().unwrap().clone();
    ...
    fn delete_first_cons(&self, mut nil_perm: Tracked<PointsTo<Nil>>, mut cons_perm: Tracked<PointsTo<Cons>>, delete_car: u32) -> (updated_nil_perm: Tracked<PointsTo<Nil>>)
        requires
            self.wf(),
            nil_perm.is_init(),
            cons_perm.is_init(),
            ...
            self.wf()
    {
        ...
        if (delete_car == first_cons_view.car) {
            let updated_nil_perm = self.delete_first_cons(nil_perm, first_cons_perm, delete_car);
            self.release_lock(updated_nil_perm);
            return;
        }
        ...
        first_locked_cons.delete(first_cons_perm, delete_car);
    }
}
...
struct_with_invariants!{
    ...
    pub closed spec fn wf(&self) -> bool {
        invariant on atomic with (cell, view_car) is (v: bool, g: Option<PointsTo<Cons>>) {
            match g {
                None => v == true,
                Some(points_to) => {
                    v == false &&
...
impl LockedCons {
    fn new(car: u32, cdr: Option<Arc<LockedCons>>) -> ((cons, cons_perm): (Self, Tracked<PointsTo<Cons>>))
        requires
            cdr.is_some() ==> (
                cdr.unwrap().wf() &&
    ...
    {   
        ...
        let (cell, Tracked(perm)) = PCell::new(node);
        let atomic = AtomicBool::new(Ghost((cell, view_car)), true, Tracked(None));
        let cons = Self { atomic, cell, view_car };
    ...
    fn insert_with_cdr(&self, mut first_cons_perm: Tracked<PointsTo<Cons>>, second_cons_perm: &Tracked<PointsTo<Cons>>, insert_car: u32) -> ((updated_first_cons_perm, new_cons_perm): (Tracked<PointsTo<Cons>>, Tracked<PointsTo<Cons>>))
        requires
            self.wf(),
            ...
            insert_car < second_cons_perm.value().car
    {
        ...
        return (first_cons_perm, new_locked_cons_perm)
    }

    fn insert_with_no_cdr(&self, mut cons_perm: Tracked<PointsTo<Cons>>, insert_car: u32) -> ((updated_cons_perm, new_cons_perm): (Tracked<PointsTo<Cons>>, Tracked<PointsTo<Cons>>))
        requires
            self.wf(),
            cons_perm.is_init(),
            ...
            new_cons_perm.value().car == insert_car
    {
        ...
        return (cons_perm, new_locked_cons_perm)
    }
            ...
            self.wf()
    {
                ...
                insert_car - current_cons_perm.value().car
        {
            ...
            if (current_cons_view.cdr.is_none()) {
                let (mut updated_current_cons_perm, new_cons_perm) = current_locked_cons.insert_with_no_cdr(current_cons_perm, insert_car);
            ...
            // Otherwise, there is another LockedCons
                ...
                if (insert_car < next_cons_view.car) {
                    ...
                    let (mut updated_current_cons_perm, new_cons_perm) = current_locked_cons.insert_with_cdr(current_cons_perm, &next_cons_perm, insert_car);
                    let updated_current_cons_view = current_locked_cons.cell.borrow(Tracked(updated_current_cons_perm.borrow_mut()));
                    let new_locked_cons = updated_current_cons_view.cdr.as_ref().unwrap().clone();
    ...
    fn delete_cons(&self, mut first_cons_perm: Tracked<PointsTo<Cons>>, mut delete_cons_perm: Tracked<PointsTo<Cons>>, delete_car: u32) -> (updated_first_cons_perm: Tracked<PointsTo<Cons>>)
        requires
            self.wf(),
            first_cons_perm.is_init(),
    ...
    fn delete(self: Arc<Self>, mut current_cons_perm: Tracked<PointsTo<Cons>>, delete_car: u32)
        requires
            self.wf(),
            current_cons_perm.is_init(),
            current_cons_perm.id() == self.cell.id(),
            ...
            self.wf()
    {
                ...
                delete_car - current_cons_perm.value().car
        {
            ...
            // Otherwise, there is another LockedCons
                ...
                // Check if we are deleting the first LockedCons:
                if (delete_car == next_cons_view.car) {
                    let updated_current_cons_perm = current_locked_cons.delete_cons(current_cons_perm, next_cons_perm, delete_car);
                    current_locked_cons.release_lock(updated_current_cons_perm);
                    return;
                }
