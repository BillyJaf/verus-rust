...
tokenized_state_machine!{fifo_queue<T> {
    fields {
        ...
        pub storage: Map<nat, cell::pcell_maybe_uninit::PointsTo<T>>,
    ...
    init!{
        initialize(backing_cells: Seq<CellId>, storage: Map<nat, cell::pcell_maybe_uninit::PointsTo<T>>) {
    ...
    transition!{
        ...
        produce_end(perm: cell::pcell_maybe_uninit::PointsTo<T>) {
    ...
    transition!{
        consume_end(perm: cell::pcell_maybe_uninit::PointsTo<T>) {
    ...
    fn initialize_inductive(post: Self, backing_cells: Seq<CellId>, storage: Map<nat, cell::pcell_maybe_uninit::PointsTo<T>>) {
        assert forall|i: nat|
    ...
    fn produce_end_inductive(pre: Self, post: Self, perm: cell::pcell_maybe_uninit::PointsTo<T>) {
        assert forall |i|
    ...
    fn consume_end_inductive(pre: Self, post: Self, perm: cell::pcell_maybe_uninit::PointsTo<T>) {
        let head = pre.consumer.get_Consuming_0();
...
struct_with_invariants!{
    struct Queue<T> {
        buffer: Vec<PCell<T>>,
        head: AtomicU64<_, fifo_queue::head<T>, _>,
        tail: AtomicU64<_, fifo_queue::tail<T>, _>,

        instance: Tracked<fifo_queue::Instance<T>>,
    }
    ...
    pub closed spec fn wf(&self) -> bool {
        predicate {
            ...
            &&& forall|i: int| #![auto] 0 <= i && i < self.buffer@.len() as int ==>
                self.instance@.backing_cells().index(i) ===
                    self.buffer@.index(i).id()
        }

        invariant on head with (instance) is (v: u64, g: fifo_queue::head<T>) {
            &&& g.instance_id() === instance@.id()
            &&& g.value() == v as int
        }

        invariant on tail with (instance) is (v: u64, g: fifo_queue::tail<T>) {
            &&& g.instance_id() === instance@.id()
            &&& g.value() == v as int
        }
    }
...
pub struct Producer<T> {
    queue: Arc<Queue<T>>,
    tail: usize,
    producer: Tracked<fifo_queue::producer<T>>,
}
...
pub struct Consumer<T> {
    queue: Arc<Queue<T>>,
    head: usize,
    consumer: Tracked<fifo_queue::consumer<T>>,
}
        ...
        pc.1.wf(),
{
    ...
    let tracked mut perms = Map::<nat, cell::pcell_maybe_uninit::PointsTo<T>>::tracked_empty();
    ...
    ) = fifo_queue::Instance::initialize(backing_cells_ids, perms, perms);
    // Initialize atomics
    let tracked_inst: Tracked<fifo_queue::Instance<T>> = Tracked(instance.clone());
...
impl<T> Producer<T> {
            ...
            self.wf(),
    {
                ...
                self.wf(),
        {
            ...
            let tracked cell_perm: Option<cell::pcell_maybe_uninit::PointsTo<T>>;
