...
tokenized_state_machine!{
    machine {
        fields {
            // Map of node ids to the data that they hold
            #[sharding(map)]
...
impl LockedDummyNode {
            ...
            locked_dummy_node.wf()
    {
        ...
        let node = DummyNode { head: None::<Arc<LockedNode>>};
...
impl LockedNode {
    fn new(data: u32, next_node: Option<Arc<LockedNode>>, instance: Tracked<machine::Instance>) -> (new_node: Self)
        requires
            next_node.is_some() ==> (
                next_node.unwrap().wf() &&
                next_node.unwrap().instance == instance &&
                data < next_node.unwrap().data_view
            )
        ensures 
            new_node.wf(),
    ...
    {   
        ...
        let (cell, Tracked(perm)) = pcell::PCell::new(node);
        let atomic = AtomicBool::new(Ghost((cell, instance, data)), false, Tracked(Some(perm)));
