    ...
    machine {
        ...
        pub fn all_stack_cell_permissions_witnesses_contains_next_witness_inv(&self) -> bool {
            forall |addr: StackCellAddress| #![auto]
                ...
                self.all_stack_cell_permissions_witnesses.dom().contains(self.all_stack_cell_permissions_witnesses.index(addr).value().next)
        }
        ...
        transition!{
            push(points_to: PointsTo<StackCell>)
            {
                ...
                require(!pre.all_stack_cell_addresses.contains(points_to.addr()));
                require(pre.all_stack_cell_addresses.contains(points_to.value().next));

                require(pre.current_stack_cell_addresses.last() == points_to.value().next);

                update all_stack_cell_addresses                = pre.all_stack_cell_addresses.insert(points_to.addr());
                update current_stack_cell_addresses      = pre.current_stack_cell_addresses.push(points_to.addr());
                update current_stack_cell_elements      = pre.current_stack_cell_elements.push(StackCellContents::Elem(points_to.value().elem));
                deposit all_stack_cell_address_permissions             += [points_to.addr() => points_to];
                add all_stack_cell_permissions_witnesses (union)= [points_to.addr() => points_to];
        ...
        transition!{
            pop(second_top_points_to: PointsTo<StackCell>, top_points_to: PointsTo<StackCell>)
            {
                ...
                require(pre.current_stack_cell_addresses.last() == top_points_to.addr());
                require(top_points_to.value().next == second_top_points_to.addr());

                have all_stack_cell_permissions_witnesses >= [top_points_to.addr() => top_points_to];
        ...
        transition!{
            empty_stack_pop(points_to: PointsTo<StackCell>)
            {
                ...
                have all_stack_cell_permissions_witnesses >= [points_to.addr() => points_to];
        ...
        property!{
            get_permission_reference(addr: StackCellAddress, permission: PointsTo<StackCell>) {
                have all_stack_cell_permissions_witnesses >= [addr => permission];
                guard all_stack_cell_address_permissions          >= [addr => permission];
            }
        }

        property!{
            have_witness_after_pop(addr: StackCellAddress, permission: PointsTo<StackCell>) {
                have all_stack_cell_permissions_witnesses >= [addr => permission];
                require(addr != pre.base_address);
                assert(pre.all_stack_cell_addresses.contains(permission.value().next));
            }
        }

        property!{
            same_address_implies_same_permission(addr1: StackCellAddress, permission1: PointsTo<StackCell>, addr2: StackCellAddress, permission2: PointsTo<StackCell>) {
                require(addr1 == addr2);
                have all_stack_cell_permissions_witnesses >= [addr1 => permission1];
                have all_stack_cell_permissions_witnesses >= [addr2 => permission2];
        ...
        fn push_inductive(pre: Self, post: Self, points_to: PointsTo<StackCell>) {
            assert(post.current_stack_cell_addresses.first() == post.base_address);
            assert(post.current_stack_cell_addresses.contains(post.base_address));

            assert(
                forall |addr: StackCellAddress|
                    #[trigger] pre.current_stack_cell_addresses.contains(addr) ==> (
                        forall |i: int|
                            (0 < i < pre.current_stack_cell_addresses.len() && #[trigger] pre.current_stack_cell_addresses[i] == addr) ==> (
                                pre.current_stack_cell_addresses[i-1] == pre.all_stack_cell_permissions_witnesses.index(addr).value().next
                            )
                    )
            );

            assert(pre.current_stack_cell_addresses == post.current_stack_cell_addresses.subrange(0, post.current_stack_cell_addresses.len() - 1));

            assert(
                forall |addr: StackCellAddress|
                    #[trigger] post.current_stack_cell_addresses.subrange(0, post.current_stack_cell_addresses.len() - 1).contains(addr) ==> (
                        forall |i: int|
                            (0 < i < post.current_stack_cell_addresses.subrange(0, post.current_stack_cell_addresses.len() - 1).len() && #[trigger] post.current_stack_cell_addresses.subrange(0, post.current_stack_cell_addresses.len() - 1)[i] == addr) ==> (
                                post.current_stack_cell_addresses.subrange(0, post.current_stack_cell_addresses.len() - 1)[i-1] == post.all_stack_cell_permissions_witnesses.index(addr).value().next
                            )
                    )
            );
            ...
            assert(
                forall |addr: StackCellAddress|
                    #[trigger] post.current_stack_cell_addresses.subrange(0, post.current_stack_cell_addresses.len() - 1).contains(addr) ==> (
                        forall |i: int|
                            (0 < i < post.current_stack_cell_addresses.len() && #[trigger] post.current_stack_cell_addresses[i] == addr) ==> (
                                post.current_stack_cell_addresses[i-1] == post.all_stack_cell_permissions_witnesses.index(addr).value().next
                            )
                    )
            );

            assert(pre.current_stack_cell_addresses.push(points_to.addr()) == post.current_stack_cell_addresses);
            assert(post.all_stack_cell_permissions_witnesses.contains_key(points_to.addr()));

            assert(post.current_stack_cell_addresses.last() == (points_to.addr()));
            assert(post.current_stack_cell_addresses.contains(points_to.addr()));

            assert(
                forall |addr: StackCellAddress|
                    (#[trigger] post.current_stack_cell_addresses.contains(addr) && addr != post.current_stack_cell_addresses.last()) ==>
                        (post.current_stack_cell_addresses.subrange(0, post.current_stack_cell_addresses.len() - 1).contains(addr))
            );


            assert(pre.current_stack_cell_addresses.to_set().union(pre.popped_stack_cell_addresses) == pre.all_stack_cell_addresses);
            assert(post.current_stack_cell_addresses.to_set().union(post.popped_stack_cell_addresses) == post.all_stack_cell_addresses);
        ...
        fn pop_inductive(pre: Self, post: Self, second_top_points_to: PointsTo<StackCell>, top_points_to: PointsTo<StackCell>) {
            assert(pre.current_stack_cell_addresses.first() == post.base_address);
            assert(post.current_stack_cell_addresses.first() == post.base_address);
            ...
            assert(
                forall |addr: StackCellAddress|
                    #[trigger] pre.current_stack_cell_addresses.contains(addr) ==> (
                        forall |i: int|
                            (0 < i < pre.current_stack_cell_addresses.drop_last().len() && #[trigger] pre.current_stack_cell_addresses.drop_last()[i] == addr) ==> (
                                pre.current_stack_cell_addresses.drop_last()[i-1] == pre.all_stack_cell_permissions_witnesses.index(addr).value().next
                            )
                    )
            );
            ...
            assert(
                forall |addr: StackCellAddress|
                    #[trigger] pre.current_stack_cell_addresses.drop_last().contains(addr) ==>
                        pre.current_stack_cell_addresses.contains(addr)
            );

            assert(pre.all_stack_cell_addresses == post.all_stack_cell_addresses);
            assert(pre.current_stack_cell_addresses.to_set().union(pre.popped_stack_cell_addresses) == pre.all_stack_cell_addresses);

            assert(post.current_stack_cell_addresses == pre.current_stack_cell_addresses.drop_last());
            assert(post.popped_stack_cell_addresses == pre.popped_stack_cell_addresses.insert(pre.current_stack_cell_addresses.last()));
        ...
        fn empty_stack_pop_inductive(pre: Self, post: Self, points_to: PointsTo<StackCell>) {
            assert(replay_history(0nat, pre.linearised_history, Seq::empty()) == pre.current_stack_cell_elements);
...
pub struct AtomicTokens {
    ...
    pub cell_witnesses: Tracked<Map<StackCellAddress, machine::all_stack_cell_permissions_witnesses>>,
        ...
        invariant on top_address with (base_address, instance) is (top_address: usize, atomic_tokens: AtomicTokens) {
            ...
            &&& (forall |addr: StackCellAddress| #![auto]
                    atomic_tokens.cell_witnesses.dom().contains(addr) ==>
                        atomic_tokens.cell_witnesses.index(addr).instance_id() == instance.id())

            // The top address is always tracked:
            &&& atomic_tokens.cell_witnesses.dom().contains(top_address)
            &&& atomic_tokens.current_stack_cell_addresses.value().contains(top_address)
            &&& atomic_tokens.current_stack_cell_addresses.value().last() == top_address

            // The set of cell addresses should equal the domain of the witness tokens:
            &&&  atomic_tokens.all_stack_cell_addresses.value() == atomic_tokens.cell_witnesses.dom()

            // Every witness token's permission points to initialised memory except for the witness of the base address:
            &&& (forall |addr: StackCellAddress| #![auto]
                    atomic_tokens.cell_witnesses.dom().contains(addr) ==> (
                        addr != base_address <==> atomic_tokens.cell_witnesses.index(addr).value().is_init()
                    ))

            // Each individual map entry must agree internally at the address it is referencing
            &&& (forall |addr: StackCellAddress| #![auto]
                    atomic_tokens.cell_witnesses.dom().contains(addr) ==> (
                        atomic_tokens.cell_witnesses.index(addr).key() == addr &&
                        atomic_tokens.cell_witnesses.index(addr).value().addr() == addr
                    ))

            // TESTING:
            &&& atomic_tokens.current_stack_cell_addresses.value().to_set().subset_of(atomic_tokens.all_stack_cell_addresses.value())
            ...
            &&& forall |addr: StackCellAddress|
                    #[trigger] atomic_tokens.current_stack_cell_addresses.value().contains(addr) ==> (
                        forall |i: int|
                            (0 < i < atomic_tokens.current_stack_cell_addresses.value().len() && #[trigger] atomic_tokens.current_stack_cell_addresses.value()[i] == addr) ==> (
                                atomic_tokens.current_stack_cell_addresses.value()[i-1] == atomic_tokens.cell_witnesses.index(addr).value().value().next
                            )
                    )
            &&& top_address == base_address <==> atomic_tokens.current_stack_cell_addresses.value().len() == 1
            // &&& atomic_tokens.current_stack_cell_addresses.value().len() == 1 ==> atomic_tokens.current_stack_cell_addresses.value().first() == atomic_tokens.current_stack_cell_addresses.value().last()
        }
    }
}
            ...
            let mut push_result =
                ...
                ghost points_to_inv => {
                    if let Ok(previous_head) = previous_head_result {
                        ...
                        assert(points_to_inv.current_stack_cell_addresses.value().first() == self.base_address);

                        assert(
                            forall |addr: StackCellAddress|
                                #[trigger] points_to_inv.current_stack_cell_addresses.value().contains(addr) ==> (
                                    forall |i: int|
                                        (0 < i < points_to_inv.current_stack_cell_addresses.value().len() && #[trigger] points_to_inv.current_stack_cell_addresses.value()[i] == addr) ==> (
                                            points_to_inv.current_stack_cell_addresses.value()[i-1] == points_to_inv.cell_witnesses.index(addr).value().value().next
                                        )
                                )
                        );
                        ...
                        let tracked tuple = self.instance.push(stack_cell_perm, &mut points_to_inv.current_stack_cell_elements, &mut points_to_inv.current_stack_cell_addresses, &mut points_to_inv.all_stack_cell_addresses, stack_cell_perm);

                        assert(points_to_inv.current_stack_cell_addresses.value().first() == self.instance.base_address());
                        assert(points_to_inv.current_stack_cell_addresses.value().last() == next);
                        ...
                        assert(points_to_inv.current_stack_cell_addresses.value() == pre_current_stack_cell_addresses.push(witness_token.value().addr()));
                        assert(points_to_inv.current_stack_cell_addresses.value().drop_last() == pre_current_stack_cell_addresses.push(witness_token.value().addr()).drop_last());
                        assert(pre_current_stack_cell_addresses@ =~= pre_current_stack_cell_addresses.push(witness_token.value().addr()).drop_last());
                        assert(points_to_inv.current_stack_cell_addresses.value().drop_last() == pre_current_stack_cell_addresses);

                        assert(
                            forall |addr: StackCellAddress|
                                #[trigger] pre_current_stack_cell_addresses.contains(addr) ==> (
                                    forall |i: int|
                                        (0 < i < pre_current_stack_cell_addresses.len() && #[trigger] pre_current_stack_cell_addresses[i] == addr) ==> (
                                            pre_current_stack_cell_addresses[i-1] == points_to_inv.cell_witnesses.index(addr).value().value().next
                                        )
                                )
                        );

                        assert(points_to_inv.current_stack_cell_addresses.value().last() == witness_token.key());
                        assert(points_to_inv.current_stack_cell_addresses.value()[points_to_inv.current_stack_cell_addresses.value().len() - 2] == witness_token.value().value().next);
                        ...
                        assert(
                            forall |addr: StackCellAddress|
                                #[trigger] pre_current_stack_cell_addresses.contains(addr) ==> (
                                    forall |i: int|
                                        (0 < i < pre_current_stack_cell_addresses.len() - 1 && #[trigger] pre_current_stack_cell_addresses[i] == addr) ==> (
                                            pre_current_stack_cell_addresses[i-1] == points_to_inv.cell_witnesses.index(addr).value().value().next
                                        )
                                )
                        );
                        ...
                        assert(
                            forall |addr: StackCellAddress|
                                #[trigger] points_to_inv.current_stack_cell_addresses.value().subrange(0, points_to_inv.current_stack_cell_addresses.value().len() - 1).contains(addr) ==>
                                    pre_current_stack_cell_addresses.contains(addr)
                        );

                        assert(pre_current_stack_cell_addresses.push(witness_token.value().addr()) == points_to_inv.current_stack_cell_addresses.value());
                        assert(pre_current_stack_cell_addresses == points_to_inv.current_stack_cell_addresses.value().subrange(0, points_to_inv.current_stack_cell_addresses.value().len() - 1));

                        assert(
                            forall |addr: StackCellAddress|
                                #[trigger] points_to_inv.current_stack_cell_addresses.value().subrange(0, points_to_inv.current_stack_cell_addresses.value().len() - 1).contains(addr) ==> (
                                    forall |i: int|
                                        (0 < i < points_to_inv.current_stack_cell_addresses.value().subrange(0, points_to_inv.current_stack_cell_addresses.value().len() - 1).len() - 1 && #[trigger] points_to_inv.current_stack_cell_addresses.value().subrange(0, pre_current_stack_cell_addresses.len() - 1)[i] == addr) ==> (
                                            points_to_inv.current_stack_cell_addresses.value().subrange(0, points_to_inv.current_stack_cell_addresses.value().len() - 1)[i-1] == points_to_inv.cell_witnesses.index(addr).value().value().next
                                        )
                                )
                        );

                        assert(points_to_inv.cell_witnesses.contains_key(witness_token.key()));
                        // assert(points_to_inv.current_stack_cell_addresses.value().subrange(0, pre_current_stack_cell_addresses.len() - 1).push(witness_token.key()) =~= points_to_inv.current_stack_cell_addresses.value());

                        assert(points_to_inv.current_stack_cell_addresses.value().last() == witness_token.key());
                        assert(points_to_inv.current_stack_cell_addresses.value().contains(witness_token.key()));

                        assert(
                            forall |i: int|
                                (0 < i < points_to_inv.current_stack_cell_addresses.value().len() - 1 && #[trigger] points_to_inv.current_stack_cell_addresses.value()[i] == witness_token.key()) ==> (
                                    points_to_inv.current_stack_cell_addresses.value()[i-1] == points_to_inv.cell_witnesses.index(witness_token.key()).value().value().next
                                )
                        );
                        ...
                        assert(
                            forall |addr: StackCellAddress|
                                (#[trigger] points_to_inv.current_stack_cell_addresses.value().contains(addr) && addr != points_to_inv.current_stack_cell_addresses.value().last()) ==>
                                    (points_to_inv.current_stack_cell_addresses.value().subrange(0, points_to_inv.current_stack_cell_addresses.value().len() - 1).contains(addr))
                        );
                    }
                }
            );
                ...
                self.wf(),
        {
            ...
            let mut head_stack_cell_address =
                ...
                ghost points_to_inv => {
                    ...
                    if addr == self.base_address {
                        pop_witness = Some(self.instance.empty_stack_pop(head_witness_token.value(), &mut points_to_inv.current_stack_cell_addresses, &head_witness_token).1.get());
                    }
                }
            };
            ...
            if head_stack_cell_address == self.base_address {
                return PoppedElemAndWitness {
                    elem: None,
                    witness: Tracked(pop_witness.tracked_unwrap()),
                };
            }
            proof {
                token_ref =
                self.instance.get_permission_reference(
                    head_witness_token.key(),
                    head_witness_token.value(),
                    &head_witness_token,
                );
            }
            ...
            let mut possible_new_head_address =
                atomic_with_ghost!{
                self.top_address => compare_exchange(
                ...
                ghost points_to_inv => {
                    if let Ok(_) = possible_new_head_address {
                        ...
                        self.instance.have_witness_after_pop(head_witness_token.key(), head_witness_token.value(), &points_to_inv.all_stack_cell_addresses, &head_witness_token);
                        ...
                        assert(points_to_inv.current_stack_cell_addresses.value().last() == current_head_address);
                        ...
                        assert(points_to_inv.current_stack_cell_addresses.value()[points_to_inv.current_stack_cell_addresses.value().len() - 1] == current_head_address);
                        assert(points_to_inv.current_stack_cell_addresses.value()[points_to_inv.current_stack_cell_addresses.value().len() - 2] == points_to_inv.cell_witnesses.index(current_head_address).value().value().next);
                        ...
                        let ghost post_current_stack_cell_addresses = Ghost(points_to_inv.current_stack_cell_addresses@.value().drop_last());
                        assert(post_current_stack_cell_addresses == pre_current_stack_cell_addresses.subrange(0, pre_current_stack_cell_addresses.len() - 1));

                        assert(
                            forall |addr: StackCellAddress|
                                #[trigger] pre_current_stack_cell_addresses.contains(addr) ==> (
                                    forall |i: int|
                                        (0 < i < pre_current_stack_cell_addresses.subrange(0, pre_current_stack_cell_addresses.len() - 1).len() && #[trigger] pre_current_stack_cell_addresses.subrange(0, pre_current_stack_cell_addresses.len() - 1)[i] == addr) ==> (
                                            pre_current_stack_cell_addresses.subrange(0, pre_current_stack_cell_addresses.len() - 1)[i-1] == points_to_inv.cell_witnesses.index(addr).value().value().next
                                        )
                                )
                        );
                        ...
                        pop_witness = Some(self.instance.pop(new_top_witness.value(), head_witness_token.value(), &mut points_to_inv.current_stack_cell_elements, &mut points_to_inv.current_stack_cell_addresses, &mut points_to_inv.popped_stack_cell_addresses, &head_witness_token).1.get());

                        let ghost post_current_stack_cell_addresses = Ghost(points_to_inv.current_stack_cell_addresses);

                        assert(pre_current_stack_cell_addresses@.drop_last() == post_current_stack_cell_addresses.value());

                        assert(points_to_inv.current_stack_cell_addresses.value().last() == new_head_address);
